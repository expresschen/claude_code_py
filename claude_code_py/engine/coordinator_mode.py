...
Provides functionality for coordinator/orchestrator mode in Claude Code.
When enabled, Claude acts as an orchestrator that delegates work to subagents
rather than executing tasks directly.
"""

import os
from typing import Final

from claude_code_py.constants.prompts import (
    AGENT_TOOL_NAME,
    BASH_TOOL_NAME,
    FILE_READ_TOOL_NAME,
    FILE_EDIT_TOOL_NAME,
...
__all__ = [
    ...
    "get_worker_result_notification_template",
]
...
def is_coordinator_mode() -> bool:
    ...
    Returns:
        True if CLAUDE_CODE_COORDINATOR_MODE is set to "1" or "true",
        False otherwise.
    """
    value = os.environ.get("CLAUDE_CODE_COORDINATOR_MODE", "")
...
def get_coordinator_system_prompt() -> str:
    """
    Get the system prompt for coordinator/orchestrator mode.

    This prompt instructs Claude to act as an orchestrator that delegates
    work to workers rather than executing tasks directly.

    Returns:
        The coordinator system prompt string.
    """
    ...
    worker_capabilities = (
        ...
        if is_simple
        else "Workers have access to standard tools, MCP tools from configured MCP servers, and project skills via the Skill tool. Delegate skill invocations (e.g. /commit, /verify) to workers."
    )
...
When calling {AGENT_TOOL_NAME}, use subagent_type `general-purpose` (or omit it for the default). Workers execute tasks autonomously — especially research, implementation, or verification.

For specialized tasks:
- `Explore` — read-only codebase exploration, fast searches
- `Plan` — architecture analysis, implementation planning (read-only)
- `verification` — test and verify implementation work
...
def get_worker_result_notification_template() -> str:
    """
    Get the XML template for worker result notifications.

    This template defines the format for task-notification messages that
    workers send back to the coordinator upon completion.

    Returns:
        XML template string with placeholders for agent_id, status,
        summary, result, and usage.
    """
