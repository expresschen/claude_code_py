...
def get_model_max_output_tokens(model: str) -> dict[str, int]:
    ...
    Based on TypeScript's getModelMaxOutputTokens in utils/context.ts.

    Args:
        model: Model name

    Returns:
        Dict with 'default' and 'upper_limit' keys
    """
    from claude_code_py.utils.model import get_canonical_name

    canonical = get_canonical_name(model)

    # Claude 4.6 models
    if "opus-4-6" in canonical:
        return {"default": 64_000, "upper_limit": 128_000}
    elif "sonnet-4-6" in canonical:
        return {"default": 32_000, "upper_limit": 128_000}
    # Claude 4.5 / 4.0 models
    elif "opus-4-5" in canonical or "sonnet-4" in canonical or "haiku-4" in canonical:
        return {"default": 32_000, "upper_limit": 64_000}
    # Claude 4.1
    elif "opus-4-1" in canonical or "opus-4" in canonical:
        return {"default": 32_000, "upper_limit": 32_000}
    # Claude 3.x models
    elif "claude-3-opus" in canonical:
        return {"default": 4_096, "upper_limit": 4_096}
    elif "claude-3-sonnet" in canonical:
        return {"default": 8_192, "upper_limit": 8_192}
    elif "claude-3-haiku" in canonical:
        return {"default": 4_096, "upper_limit": 4_096}
    elif "3-5-sonnet" in canonical or "3-5-haiku" in canonical:
        return {"default": 8_192, "upper_limit": 8_192}
    elif "3-7-sonnet" in canonical:
        return {"default": 32_000, "upper_limit": 64_000}
    else:
        return {"default": MAX_OUTPUT_TOKENS_DEFAULT, "upper_limit": MAX_OUTPUT_TOKENS_UPPER_LIMIT}
...
def get_max_thinking_tokens_for_model(model: str) -> int:
    ...
    """
    upper_limit = get_model_max_output_tokens(model).get("upper_limit", 128_000)
